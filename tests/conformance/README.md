...
| **`parquet.json`** | **parquet** | **17** | **wispdf/parquet subpath (ADR-011): 11 round-trip cases (all 9 supported dtypes + multi-dtype frame + Snappy); 6 out-of-profile error cases (gzip, zstd, INT96 timestamp, DECIMAL, LIST nested, invalid write codec). parquet-wasm devDep oracle for write validation and out-of-profile file generation.** | **v2 (dh9.2)** |
