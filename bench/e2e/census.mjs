 ...
 *   4. derive      — state = GEOID.slice(0, 11)  (escape hatch: mapFn → withColumn)
...
import { DataFrame, fromArrow, runtimeFromExports, useRuntime } from '../../dist/index.js';
...
async function main() {
  ...
  // Escape hatch: str namespace not yet available → mapFn + withColumn
  ...
  const states = dfJoined.mapFn(row => row.GEOID.slice(0, 11));
  const dfWithState = dfJoined.withColumn('state', states, { dtype: 'utf8' });
  ...
  const steps = [
    ...
    { name: 'derive state',   db: db_derive,      aq: aq_derive    },
    { name: 'groupby sum',    db: db_groupby,     aq: aq_groupby   },
  ];

  // Totals (excluding init runtime for fair comparison against user measurement)
  ...
  const hdr = ['step'.padEnd(20), 'databonk(ms)'.padStart(14), 'arquero(ms)'.padStart(12), 'ratio(db/aq)'.padStart(13)];
  console.log(hdr.join(''));
  console.log('-'.repeat(59));

  for (const { name, db, aq: aqMs } of steps) {
    const ratio = aqMs > 0 ? (db / aqMs).toFixed(2) + '×' : 'n/a';
    console.log([
      name.padEnd(20),
      db.toFixed(1).padStart(14),
      (aqMs > 0 ? aqMs.toFixed(1) : 'n/a').padStart(12),
      ratio.padStart(13),
    ].join(''));
  }
  ...
  console.log('-'.repeat(59));
  console.log([
    'TOTAL (excl init)'.padEnd(20),
    dbTotal.toFixed(1).padStart(14),
  ...
  if (writeJson) {
    const baseline = {
      meta: {
        ...
        note: 'CP.0 baseline — databonk vs Arquero census pipeline',
      },
      steps: {},
    };
    for (const { name, db, aq: aqMs } of steps) {
      baseline.steps[name] = {
        ...
        arquero_ms:  aqMs > 0 ? +aqMs.toFixed(2) : null,
        ratio:       aqMs > 0 ? +(db / aqMs).toFixed(3) : null,
      };
    }
